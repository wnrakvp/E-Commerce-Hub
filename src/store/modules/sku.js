  ...
  actions: {
    getAll ({commit}) {
      return api.getAllSKU().then(({result}) => {
        const skuList = []
        result.forEach(({_id, product, name, desc, price, image, marketplaces}) => {
    ...
    get (context, id) {
      return api.getSKU(id).then(({result}) => {
        let {_id, product, name, desc, price, image, marketplaces} = result
    ...
    draft () {
      return Promise.resolve(new SKUModel('', '', '', '', 0,'https://www.fenwick.co.uk/dw/image/v2/BBKK_PRD/on/demandware.static/-/Sites-fenwick-master-catalog/default/dw46146d85/images/large/0000102333.jpg', new Set(['shopee', 'lazada'])))
    },
