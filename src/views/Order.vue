...
export default { 
  mounted () {
   ...
   watch: {
    ordertList: {
      handler(newValue) {
        console.warn('New orderList are:',newValue)
      },
      deep: true,
    },
  },
  ...
  methods: {      
    ...
    getCountItems(i){
     ...
     getCountStatus(statevalue) {
       ...
       for(let i = 0; i < this.orderList.length; i++){
        ...
        // console.log('*** orderList ***: '+this.orderList[i].orderNo)
        // console.log('orderStatus: '+this.orderList[i].orderStatus)
        // console.log('price: '+this.orderList[i].price)

            for (let j = 0; j < item.length; j++) {
              console.log('price: '+ item[j].price)
            }
    ...
    if (statevalue === 'READY_TO_SHIP')
     ...
     Logger() {
      console.log('*** Logger **** orderList:'+this.orderList);
      console.log('orderNo:'+this.orderList[0]);
