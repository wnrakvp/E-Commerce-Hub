<template>
  <div class="my-3 p-3 bg-body rounded shadow-sm">
    <div
      class="d-flex justify-content-between align-items-center border-bottom pb-1"
    >
      <h6 class="h6 my-0">Order Management</h6>

      <button class="btn btn-sm btn-outline-primary" @click="fetchALL">
        <i class="bi-arrow-repeat" style="font-size: 20px"></i> Fetch
      </button>
    </div>

    <div class="table-responsive">
      <table class="table text-center align-middle">
        <thead>
          <tr>
            <th>
              <button
                class="btn btn-sm btn-outline-primary border-0"
                value="All"
                @click="filterByOrderStatus(null)"
              >
                <i></i><span>Total({{ getCountTotal() }})</span>
              </button>
            </th>
            <th>
              <button
                class="btn btn-sm btn-outline-danger border-0"
                value="READY_TO_SHIP"
                @click="filterByOrderStatus('READY_TO_SHIP')"
              >
                <i></i>Ready to ship({{ getCountStatus('READY_TO_SHIP') }})
              </button>
            </th>
            <th>
              <button
                class="btn btn-sm btn-outline-warning border-0"
                value="ON_DELIVERY"
                @click="filterByOrderStatus('ON_DELIVERY')"
              >
                <i></i>On Delivery({{ getCountStatus('ON_DELIVERY') }})
              </button>
            </th>
            <th>
              <button
                class="btn btn-sm btn-outline-info border-0"
                value="SHIPPED"
                @click="filterByOrderStatus('SHIPPED')"
              >
                <i></i>Shipped({{ getCountStatus('SHIPPED') }})
              </button>
            </th>
            <th>
              <button
                class="btn btn-sm btn-outline-success border-0"
                value="COMPLETED"
                @click="filterByOrderStatus('COMPLETED')"
              >
                <i></i>Completed({{ getCountStatus('COMPLETED') }})
              </button>
            </th>
          </tr>
        </thead>
        <thead>
          <tr>
            <th scope="col">Marketplace</th>
            <th scope="col">Order Date</th>
            <th scope="col">Order No</th>
            <th scope="col">Tracking</th>
            <th scope="col">Courier</th>
            <th scope="col">Item</th>
            <th scope="col">Total</th>
            <th scope="col">Status</th>
            <th scope="col">View</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(order, i) in orderList" :key="i">
            <td>
              <img
                v-if="order.marketplace.toLowerCase() === 'shopee'"
                src="data:image/png;base64,"
                alt="shopee"
                width="24"
                height="24"
              />
              <img
                v-if="order.marketplace.toLowerCase() === 'lazada'"
                src="data:image/jpeg;base64,"
                alt="lazada"
                width="24"
                height="24"
              />
              <span class="badge rounded-pill bg-light text-dark">{{ order.marketplace }}</span>
            </td>
            <td><span class="badge rounded-pill bg-light text-dark">{{ formatDate(order.date) }}</span></td>
            <td>{{ order.orderNo }}</td>
            <td>{{ order.trackNo }}</td>
            <td>{{ order.courier }}</td>
            <td>{{ getCountItems(i) }}</td>
            <td>{{ getTotal(i).toLocaleString() }}</td>
            <td>
              <span
                class="badge rounded-pill"
                :class="{ 'bg-warning': order.orderStatus === 'ON_DELIVERY',
                          'bg-danger': order.orderStatus === 'READY_TO_SHIP',
                          'bg-info': order.orderStatus === 'SHIPPED',
                          'bg-success': order.orderStatus === 'COMPLETED',
                          }"
                >{{ getTextStatus(order.orderStatus) }}</span
              >
            </td>
            <td>
              <router-link
                :to="{ name: 'order-item', params: { id: order.id } }"
                custom
                v-slot="{ navigate }"
              >
                <button
                  class="btn btn-sm btn-outline-secondary"
                  @click="navigate"
                >
                  <i class="bi-three-dots"></i>
                </button>
              </router-link>
            </td>
          </tr>
        </tbody>
      </table>
    </div>
  </div>

  <div class="overflow-auto">
    <b-pagination
      v-model="currentPage"
      :total-rows="rows"
      :per-page="perPage"
      aria-controls="my-table"
    ></b-pagination>

    <p class="mt-3">Current Page: {{ currentPage }}</p>

    <b-table
      id="my-table"
      :items="items"
      :per-page="perPage"
      :current-page="currentPage"
      small
    ></b-table>
  </div>

  <router-view></router-view>
</template>
<script>
import { mapGetters, mapActions } from 'vuex';
export default {
  mounted() {
    Promise.all([this.getAllOrder(), this.getAllSKU()])
      .then((result) => {
        console.debug(result);
      })
      .catch(console.error);
  },
  computed: {
    ...mapGetters('Order', {
      orderList: 'all',
    }),
    ...mapGetters('SKU', {
      skuList: 'all',
    }),
  },
  data() {
    return {
      perPage: 3,
      currentPage: 1,
      items: [
        { id: 1, first_name: 'Fred', last_name: 'Flintstone' },
        { id: 2, first_name: 'Wilma', last_name: 'Flintstone' },
        { id: 3, first_name: 'Barney', last_name: 'Rubble' },
        { id: 4, first_name: 'Betty', last_name: 'Rubble' },
        { id: 5, first_name: 'Pebbles', last_name: 'Flintstone' },
        { id: 6, first_name: 'Bamm Bamm', last_name: 'Rubble' },
        { id: 7, first_name: 'The Great', last_name: 'Gazzoo' },
        { id: 8, first_name: 'Rockhead', last_name: 'Slate' },
        { id: 9, first_name: 'Pearl', last_name: 'Slaghoople' },
      ],
    };
  },
  methods: {
    ...mapActions('SKU', {
      getAllSKU: 'getAll',
    }),
    ...mapActions('Order', {
      getAllOrder: 'getAll',
      filterById: 'filterById',
      filterByOrderStatus: 'filterByOrderStatus',
    }),
    formatDate(date) {
      let d = new Date(date);
      let month = '' + (d.getMonth() + 1);
      let day = '' + d.getDate();
      let year = d.getFullYear();
      if (month.length < 2) month = '0' + month;
      if (day.length < 2) day = '0' + day;
      return [day, month, year].join('-');
    },
    getTextStatus(orderStatus) {
      var textStatus = '';
      if (orderStatus == 'READY_TO_SHIP' || orderStatus === 'ready_to_ship') {
        textStatus = 'Ready to ship';
      } else if (
        orderStatus == 'ON_DELIVERY' ||
        orderStatus === 'on_delivery'
      ) {
        textStatus = 'On delivery';
      } else if (orderStatus == 'SHIPPED' || orderStatus === 'shipped') {
        textStatus = 'Shipped';
      } else if (orderStatus == 'COMPLETED' || orderStatus === 'completed') {
        textStatus = 'Completed';
      }
      return textStatus;
    },
    getTotal(i) {
      var total = 0;
      var items = this.orderList[i].items;
      for (let j = 0; j < items.length; j++) {
        total = total + items[j].price;
      }
      return total;
    },
    getCountItems(i) {
      var CountItems = 0;
      var items = this.orderList[i].items;
      for (let j = 0; j < items.length; j++) {
        CountItems += 1;
      }
      return CountItems;
    },
    getCountTotal() {
      var count = this.orderList.length;
      return count;
    },
    getCountStatus(statevalue) {
      var count = 0;
      for (let i = 0; i < this.orderList.length; i++) {
        if (this.orderList[i].orderStatus === statevalue) {
          count = count + 1;
        }
      }
      return count;
    },
    fetchALL() {
      console.log('fetchTotal');
      Promise.all([this.getAllOrder(), this.getAllSKU()])
        .then((result) => {
          console.debug(result);
          console.log(result);
        })
        .catch(console.error);
    },
  },
};
</script>
