<template>
  <div class="my-3 p-3 bg-body rounded shadow-sm">
    ...
    >
      <h6 class="h6 my-0">Stock Management</h6>
      <router-link
        :to="{ name: 'stock-item', params: { id: 'add' } }"
        custom
        v-slot="{ navigate }"
      >
      ...
      <table class="table text-center align-middle">
        ...
        <tbody>
          <template v-for="(stock, i) in stockList" :key="i">
            <tr v-for="(item, j) in stock.items" :key="j">
              <td>{{ formatDate(stock.date) }}</td>
              <td>
                <img
                  v-if="stock.marketplace === 'shopee'"
