<template>
  <div class="my-3 p-3 bg-body rounded shadow-sm">
    <div
      class="d-flex justify-content-between align-items-center border-bottom pb-1"
    >
      <h6 class="h6 my-0">Inventory Management</h6>
      <button class="btn btn-sm btn-outline-primary ms-auto">
        <i class="bi-arrow-repeat" @click="Fetch"></i> Fetch
      </button>
      <router-link
        :to="{ name: 'inventory-item', params: { id: 'add' } }"
        custom
        v-slot="{ navigate }"
      >
        <button class="btn btn-sm btn-outline-secondary ms-3" @click="navigate">
          <i class="bi-plus-circle"></i> Add
        </button>
      </router-link>
    </div>
    <div class="table-responsive">
      <table class="table text-center align-middle">
        <thead>
          <tr>
            <!-- <th scope="col">Date</th> -->
            <th scope="col">Warehouse</th>
            <th scope="col">Marketplaces</th>
            <th scope="col" colspan="2">Product</th>
            <th scope="col">SKU</th>
            <th scope="col">Total</th>
            <th scope="col">Reserved</th>
            <th scope="col">Available</th>
            <th scope="col">Sold</th>
            <th scope="col">Views</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(item, i) in inventoryList" :key="i">
            <!-- <td></td> -->
            <td>
              <strong  v-if="item.type === 'Internal'" class="badge rounded-pill bg-warning text-dark">{{
                item.type
              }}</strong>
              <strong  v-else class="badge rounded-pill bg-info text-dark">{{
                item.type
              }}</strong>
            </td>
            <td>
              <img
                v-show="item.sku.marketplaces.has('shopee')"
                src="data:image/png;base64,"
                alt="shopee"
                width="24"
                height="24"
              />
              <img
                v-show="item.sku.marketplaces.has('lazada')"
                src="data:image/jpeg;base64,"
                alt="lazada"
                width="24"
                height="24"
              />
            </td>
            <td align="left">
              <img
                class="img-fluid rounded-start"
                style="max-width: 50px; max-height: 50px"
                :src="item.sku.product.image"
              />
            </td>
            <td align="left">{{ item.sku.product.name }}</td>
            <td align="left">
              {{ item.sku.name }}
            </td>
            <td>{{ item.amount }}</td>
            <td>{{ Reserved(item.id) - alreadySold(item.sku.id) }}</td>
            <td>
              <span
                :class="{ 'link-danger': item.amount - Reserved(item.id) < 10 }"
                >{{ item.amount - Reserved(item.id) }}</span
              >
            </td>
            <td>{{ alreadySold(item.sku.id) }}</td>
            <router-link
              :to="{
                name: 'inventory-item',
                params: {
                  id: item.id,
                },
              }"
              custom
              v-slot="{ navigate }"
            >
              <td>
                <button
                  class="btn btn-sm btn-outline-secondary"
                  @click="navigate"
                >
                  <i class="bi-three-dots"></i>
                </button>
              </td>
            </router-link>
          </tr>
        </tbody>
      </table>
    </div>
  </div>
  <router-view></router-view>
</template>
<script>
import { mapGetters, mapActions } from 'vuex';

export default {
  mounted() {
    Promise.all([
      this.getAllInventory(),
      this.getAllStock(),
      this.getAllOrders(),
    ])
      .then((result) => {
        console.log(result);
      })
      .catch(console.error);
  },
  computed: {
    ...mapGetters('Inventory', {
      inventoryList: 'all',
    }),
    ...mapGetters('Stock', {
      stockList: 'all',
    }),
    ...mapGetters('Order', {
      orderList: 'all',
    }),
  },
  data() {
    return {};
  },
  methods: {
    ...mapActions('Inventory', {
      getAllInventory: 'getAll',
    }),
    ...mapActions('Stock', {
      getAllStock: 'getAll',
    }),
    ...mapActions('Order', {
      getAllOrders: 'getAll',
    }),
    Reserved(id) {
      var total = 0;
      var item;
      for (let i in this.stockList) {
        // console.trace();
        item = this.stockList[i].items;
        // console.log(i);
        for (let j in item) {
          // console.log(id);
          if (id == item[j].inventory._id) {
            total = total + item[j].amountonsell;
            // console.log(item[j].inventory.id)
          }
        }
      }
      return total;
    },
    alreadySold(id) {
      const items = this.orderList
        .filter((x) => x.orderStatus === 'COMPLETED')
        .map((x) => x.items);
      let itemList = [];
      items.forEach((item) => {
        item.forEach((x) => itemList.push(x));
      });
      // console.log(itemList);
      const onSold = itemList
        .filter((i) => i.skuId._id === id)
        .map((item) => item.amount);
      let sum = 0;
      for (let i in onSold) {
        sum += onSold[i];
      }
      return sum;
    },
    Fetch() {
      Promise.all([
        this.getAllInventory(),
        this.getAllStock(),
        this.getAllOrders(),
      ])
        .then((result) => {
          console.log(result);
        })
        .catch(console.error);
    },
    Logger() {
      console.log(this.stockList);
    },
  },
};
</script>
