<template>
  <div class="card shadow h-100">
    <div class="row">
      <h6 class="card-title clamp mt-3" align="center">{{ title }}</h6>
      <img
        :src="imageURL"
        :alt="title"
        class="img-fluid rounded-start mx-auto"
        style="max-width: 15rem"
      />
    </div>
    <div class="card-body">
      <!-- <p class="card-text mb-0" align="center">
          <small class="clamp">Product: {{product}}</small>
          <small class="text-muted clamp">Attributes: {{details}}</small>
        </p>
        <p class="card-text mb-0" align="center" >
          <small class="text-muted">Price: {{price? price.toLocaleString() : '-'}} $</small>
          </p>
        <p class="card-text mb-0" align="center" >
          <small class="text-muted">Marketplace: </small>
          <small class="text-muted ">
            <img v-if="isShopee" src="data:image/png;base64," alt="shopee" width="24" height="24">
            <img v-if="isLazada" src="data:image/jpeg;base64," alt="lazada" width="24" height="24">
          </small>
        </p> -->
      <div class="table-responsive">
        <table class="table table-borderless mb-0">
          <tbody>
            <tr>
              <th scope="row" class="align-top">Product:</th>
              <td align="left">{{ product }}</td>
            </tr>
            <tr>
              <th scope="row" class="align-top">Attributes:</th>
              <td>{{ details }}</td>
            </tr>
          </tbody>
        </table>
      </div>
    </div>
    <div class="card-footer">
      <div class="d-flex align-items-center">
        <img
          v-if="isShopee"
          src="data:image/png;base64,"
          alt="shopee"
          width="36"
          height="36"
        />
        <img
          v-if="isLazada"
          src="data:image/jpeg;base64,"
          alt="lazada"
          width="36"
          height="36"
        />
        <div class="lead ms-auto">
          <strong>
            Price: {{ price ? price.toLocaleString() : "-" }}.00 $
          </strong>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
export default {
  props: {
    title: String,
    product: String,
    details: String,
    imageURL: String,
    price: Number,
    isShopee: Boolean,
    isLazada: Boolean,
  },
};
</script>
